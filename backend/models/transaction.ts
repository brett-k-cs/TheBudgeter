...
interface TransactionAttributes {
    ...
    description?: string;
    date: Date;
    accountId?: number; // Reference to an account, if applicable
...
Transaction.belongsTo(Account, { foreignKey: "accountId", onDelete: "SET NULL" });
Account.hasMany(Transaction, { foreignKey: "accountId" });

Transaction.hasMany(BudgetTransactionExclusion, {
    foreignKey: "transactionId",
    as: "exclusions",
    onDelete: "CASCADE",
});
BudgetTransactionExclusion.belongsTo(Transaction, {
    foreignKey: "transactionId",
});
...
Transaction.findAll({
    ...
    where: {
        category: {
            [Op.notIn]: Object.values(categories).map(
    ...
    .then((transactions) => {
        ...
        console.log(
            "Transactions not in predefined categories:",
            transactions.map((a) => a.toJSON())
        );
        ...
        console.log(
            "Updated transactions with labels:",
            transactions.map((a) => a.toJSON())
        );
    })
    .catch((error) => {
